...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Scan rapido!! Subscribe Fadhil Graphy'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SALVEEE @${num.split('@')[0]}\nBem vindo Ao Grupo*${mdata.subject}*\n*Fique a Vontade:)*\n\n*Leia As Regras Pra Nn Ser Removido:d E Siga robert_dsg21 No Instagram!*`
				...
				teks = `QUE PENA @${num.split('@')[0]} N FAZ MAIS PARTE DO GRUPO😕`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*[ WTF]* ⏳AGUARDE UM POUCO MANO, ENQUANTO ISSO SIGA robert_dsg21🤡...',
				success: '*[💯] Sucesso...*',
				error: {
				...
				only: {
					...
					userB: `[ ⚠️NÃO REGISTRADO⚠]\n\n️ SALVE *${pushname2}* Você ainda não esta registrado NO [BOT RD-360] \nRegistre-se primeiro Para ter acesso ao menu\n\nexemplo: ${prefix}daftar Robert/20`,
			...
			const ownerNumber = [
			"5571999926079@s.whatsapp.net"//ALTERAR NÚMERO DE LU
			]
			premium = [
			"5571999926079@s.whatsapp.net","554891886048@s.whatsapp.net"//ALTERE O NÚMERO YAG DESEJADO NO PREM, NÃO PODE FICAR NO BANCO DE DADOS !!
			]
            ...
            var persengayy = [
            `*4%*\n\n*Arrepender-se kkkk:v*`,`*9%*\n\n*OTW Grande Arrependimento:v*`,`*17%*\n\n*nmm*`,`*28%*\n\n*Hmmmm...🤦*`,`*34%*\n\n *Ta bom menos que 50%*`,`*48%*\n\n*n sei n em:v*`,`*59%*\n\n *Ta chegando perto*`,`*62%*\n\n*Quase certeza que ele e🤨*`,`*74%*\n\n*Curto broderagem n em🏃*`,`83%\n\n Fudeu mano agr tenho certeza que tu e🤦`,`97%\n\nNivel supremo Gay`,`100%\n\nKabur ae Gan Daripada Ditusbol Bool lu🏃`,`29%\n\n amann:v`,`94%\n\n Yaallah🏃`,`75%\n\nHadehh Gay🤦`,`82%\n\nMending Lu Tobat Dah🏃`,`41%\n\nSering Cari Bool Diperempatan`,`39%\n\nSering Tusbol Bool Topan🏃`
            ]
            var persenbucin = [
            `4%\n\ta bom assim🤦`,`9%\n\nSe livrou`,`17%\n\n% Ainda Pequena`,`28%\n\nYoalahh hmm`,`34%\n\nTa bom menos que 50%`,`48%\n\nNn sei...`,`59%\n\n ixi mano ta chegando perto`,`62%\n\n quase certeza que e🤨`,`74%\n\n Curto broderagem n em`,`83%\n\n Fodeu ele e Certeza" kek Ga bucin Gitu`,`97%\n\nHadehh Pakboi"`,`100%\n\nHadehhh Ini Bukan Bucin Tapi Pakboi`,`29%\n\nKasian Mana Masih Muda`,`94%\n\n Dasar Pakboi`,`75%\n\n Ya Ampun`
            ]
     ...
     const bayarLimit = (sender, amount) => {
        ...
        const limitAdd = (sender) => {
            ...
            if (args.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    reply('*[LINK DE GRUPO DETECTADO!]*\nVOU TE REMOVER DO GRUPO SEU CORNO DA PRÓXIMA VEZ PRESTE ATENÇÃO NAS REGRAS.🤡').then(() => {
                        client.groupRemove()
                    })
                }
            }
		...
		case 'antilink':
					...
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('anti link group ativado')
						antilink.push(from)
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('você já está registrado irmãzinho')
					if (isBanned) return reply(mess.only.benned)
			...
			case 'botinho':
                ...
                setTimeout( () => {
					...
					setTimeout( () => {*/
					client.sendMessage(from, 'Obrigado por usar o*RD BOT-360* \n\nSegue no insta🤡❤️https://instagram.com/robert_dsg21?igshid=cm6a38xssd85', text, {quoted: mek})
					}, 0)
    				break
/*********PISAH MENU**********/
              case 'ownermenu':
...
/*********END PISAH MENU***********/
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            .then(body => {
                ...
                .then(() => console.log('Sucesso enviando Video'))
                limitAdd(serial)
            })
            .catch(() => {
            ...
            break
				...
				case 'owner':
				case 'creator':
                 client.sendMessage(from, {displayname: "+55 71 99992-6079", vcard: vcard}, MessageType.contact, { quoted: mek})
                 client.sendMessage(from, 'Salva o ctt do meu criador ai e avisa no pv pra ele salvar tbm🤡:)',text, { quoted: mek} )
                 break
                 case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB) 
				...
				if (!isUser) return reply(mess.only.userB)
					...
					const can =['10% banyak" perawatan ya kak:v\nCanda Perawatan:v','30% Semangat Kaka Merawat Dirinya><','20% Sim espírito kaká👍','40% Wahh Kaka><','50% kaka cantik deh><','60% Hai Cantik🐊','70% Hai Ukhty🐊','62% Kakak Cantik><','74% Kakak ni cantik deh><','83% Love You Kakak><','97% Assalamualaikum Ukhty🐊','100% Kakak Pake Susuk ya??:v','29% Semangat Kakak:)','94% Hai Cantik><','75% Hai Kakak Cantik','82% wihh Kakak Pasti Sering Perawatan kan??','41% Semangat:)','39% Lebih Semangat🐊']
		    ...
		    case 'map':
			    ...
			    reply(mess.wait)
					...
					if (args[0] === 'buka') {
						...
						reply(`\`\`\`✓Grupo Fechado Com Sucesso\`\`\` *${groupMetadata.subject}*`)
				...
				case 'clearall':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, bcgc, image, {caption: `*「 TRANSMISSÃO PARA GRUPOS BY ${pushname2} 」*\n\n*Group* : ${groupName}\n*Pada Jam* :${jam}\n\n${name}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 TRANSMISSÃO PARA GRUPOS BY ${pushname2} 」*\n\n*Group* : ${groupName}\n${name}\n*Pada Jam*: ${jam}WIB\n\n${body.slice(6)}`)
						}
						reply('Grupo de transmissão de sucesso')
					}
					break
				case 'bc':
					if (!isOwner) return reply('Quem é Você Doido?')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido Recebido, @${mentioned[0].split('@')[0]} Parabéns agora você e um Admin, mantenha a ordem no grupo *${groupMetadata.subject}*`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedidos recebidos, : @${mentioned[0].split('@')[0]} Agora você e um membro comum😹`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'listadmins':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (isWelkom) return reply('Ja ativo Codorna 🐥')
						welkom.push(from)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isOwner) return reply(mess.only.ownerB)
					prefix = args[0]
					reply(`Inicial do comando alterado com sucesso para :「* ${prefix} *」`)
					break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
// Akhir Fitur Premium 
		...
		case 'jamdunia':
		 ...
		 case 'tomp3':
				...
				if (!isUser) return reply(mess.only.userB)
                	client.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('_*Convertendo para Mp3, enquanto isso siga @robert_dsg21!*_')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
				...
				case 'setppbot':
					...
					reply('Vc setou uma nova foto de perfil no Bot RD 🙂')
					break
			...
			case 'google':
               ...
               if (isBanned) return reply(mess.only.benned)    
				...
				default:
                  ...
                  reply(`Desculpe cara *${pushname2}*, Comando *${prefix}${command}* nn registrado no menu! pfv chame o bot novamente *${prefix}menu ou entre em contato com o meu dono @robert_dsg21*`)
                  }
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[RD BOT-360]','Yellow'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
